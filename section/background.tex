...
\para{\molm and \smi.}
We study two SMILES-based chemistry foundation models: \molm~\citep{ross2022largescalechemicallanguagerepresentations} and \smi~\citep{soares2024smi}. Both are transformer models pretrained using rotary linear attention without explicit geometric supervision and employ, making them a useful comparison pair for studying how different pretraining objectives and architectural choices shape internal chemical representations.

\molm is an \textit{encoder-only} model trained with a masked language modeling objective. It produces token-level hidden states and a pooled molecular representation for downstream prediction. In the original paper, \molm is evaluated on \textbf{molecular} property prediction, molecular retrieval, and quantum-chemical benchmarks including MoleculeNet, QM9, and ADMET-related tasks, where it achieves strong downstream performance. The paper additionally analyzes attention patterns and embedding structure to argue that the learned representations capture aspects of molecular 3D spatial structure and chemical similarity \citep{ross2022largescalechemicallanguagerepresentations,wu2018moleculenet}. 

\smi, by contrast, uses an \textit{encoder--decoder} design with both masked token prediction and sequence reconstruction. Its latent representation is therefore trained not only to support token-level inference, but also to retain enough information to reconstruct the input SMILES. The original \smi is evaluated on similar tasks as \molm \citep{soares2024smi,wu2018moleculenet}. \citet{sharma2025foundation} build on \smi for downstream scientific prediction and also includes an interpretability analysis by relating chemical descriptors to model behavior through correlation. 

\para{Chemical Properties and SMILES String.}
To probe learned structural representations, each \textbf{atom} is annotated with eight RDKit-derived properties. These properties should be understood as local chemical descriptors that capture chemically meaningful structural information at the atom level, rather than full molecule-level behavior. We distinguish these descriptors from geometric properties, such as interatomic distance, which are not explicitly encoded in SMILES strings.

Within the atom-level descriptors, we distinguish between \textbf{surface-form properties}, whose labels are directly readable from individual SMILES tokens, and \textbf{contextual properties}, which require the encoder to aggregate information across the molecular graph. Atom type and aromaticity are surface-form since element symbols are encoded as distinct tokens, and aromatic atoms are marked by lowercase letters in SMILES notation. By contrast, hybridization, degree, total valence, ring membership, formal charge, and chiral tag cannot be determined from individual SMILES tokens in isolation. These contextual properties therefore provide a stronger test of whether the encoder has learned chemically meaningful structure beyond surface-level tokenization patterns. Detailed definitions of all properties are provided in Appendix~\ref{appendix:properties}.
