...
\molm is an encoder-only masked language model pre-trained on molecule from PubChem \xiaoyan{cite it} and ZINC \xiaoyan{cite} \xiaoyan{Classification} via a masked language modeling objective. Its downstream applications include molecular properties prediction  \xiaoyan{What are those properties? } and multimodal battery electrolyte formulation performance prediction \xiaoyan{include other works that use Molmformer.} \citep{ross2022largescalechemicallanguagerepresentations}. \smi is a encoder-decoder foundation model pre-trained on curated SMILES from PubChem through a combined masked language modeling and token reconstruction objective, evaluated on molecular property classification and regression across eleven MoleculeNet benchmarks \citep{wu2018moleculenet} \xiaoyan{What is this benchmark? What's inside this benchmark}, SMILES reconstruction, and battery electrolyte property prediction \citep{soares2024smi}.

\para{Chemical Properties and SMILES String}

To probe learned structural representations, each atom is annotated with eight RDKit-derived \xiaoyan{cite} properties. A key distinction exists between properties whose label is determined by a single SMILES token---that is, readable directly from the written string without any contextual computation---and those that require the encoder to integrate information across the molecular graph. We call the former \textbf{surface-form properties} and the latter \textbf{contextual properties}. Atom type and aromaticity are surface-form: element symbols are encoded as distinct tokens, and aromatic atoms are marked by lowercase letters in SMILES notation, so both labels are recoverable from the embedding lookup before any transformer computation takes place. By contrast, hybridization, degree, total valence, ring membership, and chiral tag cannot be read from individual tokens in isolation; they are contextual and constitute genuine tests of what the encoder has learned beyond its tokenization. Detailed definitions of all properties are provided in Appendix~\ref{appendix:properties}.
