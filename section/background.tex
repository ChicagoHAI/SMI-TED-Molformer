...
\smi uses an \textit{encoder--decoder} design with both masked token prediction and sequence reconstruction. Its latent representation is therefore trained not only to support token-level inference, but also to retain enough information to reconstruct the input SMILES. The original \smi is evaluated on similar tasks as \molm \citep{soares2024smi,wu2018moleculenet}.

\para{Chemical Properties and SMILES Strings.} Given a molecule, each \textbf{atom} is annotated with eight properties derived from RDKit~\cite{rdkit}, an open-source cheminformatics toolkit. These properties should be understood as local chemical descriptors that capture chemically meaningful structural information at the atom level, rather than full molecule-level behavior. We distinguish these descriptors from geometric properties, such as interatomic distance, which are not explicitly encoded in SMILES strings.

Within the atom-level descriptors, we distinguish between \textbf{surface-level properties}, whose labels are directly readable from individual SMILES tokens, and \textbf{contextual properties}, which require the encoder to aggregate information across the molecular graph. Atom type and aromaticity are surface-level since element symbols are encoded as distinct tokens, and aromatic atoms are marked by lowercase letters in SMILES notation. By contrast, hybridization, degree, total valence, ring membership, formal charge, and chiral tag cannot be determined from individual SMILES tokens in isolation. These contextual properties therefore provide a stronger test of whether the encoder has learned chemically meaningful structure beyond surface-level tokenization patterns. Detailed definitions of all properties are provided in Appendix~\ref{appendix:properties}.
