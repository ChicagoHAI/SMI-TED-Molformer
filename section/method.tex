...
distance matrix $D_{ij}$ over \textbf{heavy} atoms using an
MMFF94-optimized conformer, where $i,j$ index heavy atoms,
$l$ indexes the transformer layer, and $h$ indexes the attention head.

For each $(l,h)$ pair and each molecule, we compute the
Spearman correlation between $\{A_{ij}^{(l,h)}\}$ and
$\{1/D_{ij}\}$ over all unique atom pairs $i<j$. These per-molecule correlations are then averaged across the dataset. Furthermore, we stratify atom pairs by distance range:
