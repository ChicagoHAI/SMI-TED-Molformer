...
\para{Attention-Distance Correlation Setup.} For each model and molecule, we extract per-layer, per-head
attention weights $A_{ij}^{(l,h)}$ and compute a pairwise 3D
distance matrix $D_{ij}$ over heavy atoms using an
MMFF94-optimized conformer, where $i,j$ index heavy atoms,
$l$ indexes the transformer layer, and $h$ indexes the attention head.

We then measure the Spearman correlation between
$A_{ij}^{(l,h)}$ and $1/D_{ij}$ for every $(l,h)$ pair,
averaged across molecules. Furthermore, we stratify atom pairs by distance range:
\emph{short} ($D_{ij}\le2$\,\AA),
\emph{medium} ($2<D_{ij}\le4$\,\AA), and
\emph{long} ($D_{ij}>4$\,\AA),
to identify which attention heads preferentially encode
local versus long-range spatial structure.
