...
distance matrix $D_{ij}$ over \textbf{heavy} atom pairs $(i, j)$ (more details in Appendix~\ref{app:attention}). 

For each molecule, we compute the
Spearman correlation between $\{A_{ij}^{(l,h)}\}$ and
$\{1/D_{ij}\}$ over all unique atom pairs $i<j$. These per-molecule correlations are then averaged across the dataset (see . Furthermore, we stratify atom pairs by distance range:
