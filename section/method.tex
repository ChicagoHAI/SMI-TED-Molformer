...
distance matrix $D_{ij}$ over \textbf{heavy} atom pairs, where $i, j \in \{1,\ldots,N\}$ index the $N$ heavy atoms of the molecule (more details in Appendix~\ref{app:attention-details}).  \xiaoyan{I think distance cannot be directly read from SMILES string which is different from other properties. We should probably say this somewhere.} \anita{yes this is in Appendix A}
