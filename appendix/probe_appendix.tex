...
\input{appendix/probe_acc_all}
\input{appendix/layer_steer_all}
...
\bottomrule
\end{tabular}
\caption{Layer-$11$ intervention success rate in \smi{} for the five properties that show the L11 dip. Each cell is the flip-to-target rate at the layer-$12$ probe readout under a single-layer-$11$ steering perturbation of magnitude $\alpha$. The $\alpha$ range is chosen to span below to well above the residual-stream scale: the steering vector $\mathbf{v} = (\mathbf{W}_{c'}-\mathbf{W}_{c})/\boldsymbol{\sigma}$ has Euclidean norm of order $\|\mathbf{v}\| \approx 8.8$, comparable to the per-token residual-stream norm, so $\alpha \in [1, 3]$ is approximately scale-matched and $\alpha \ge 30$ is in the regime where the next block's LayerNorm washes out further increases in magnitude. Fine-grid values $\alpha \le 1.0$ are computed on a $100$-molecule subset of held-out QM9; wide-grid values $\alpha \ge 3$ are computed on the main-sweep $200$-molecule subset. Reference rows show the best success rate over the wide $\alpha$ grid at the adjacent layers L10 and L12. None of the L11 values approach the L10 / L12 ceilings at any tested magnitude, ruling out a magnitude artifact.}
\label{app:l11-table}
\end{table}
